...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val others = left.union(right)
    if (others.isEmpty) elem
    else {
      lazy val best = others.mostRetweeted
      if (elem.retweets > best.retweets) elem else best
    }
  }
...
trait TweetList {
  ...
  override def toString() : String =
    head.user+"," + head.retweets + "," +tail.toString()
}
...
object GoogleVsApple {
  ...
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter( x => apple.exists( word => x.text.contains(word)))
